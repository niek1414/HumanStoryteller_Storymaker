...
export default draw2d.command.Command.extend({
  ...
  undo: function () {
    ...
    for (let i = 0; i < this.figure.getChildren().getSize(); ++i) {
      const item = this.figure.getChildren().get(i);
      if (item instanceof draw2d.shape.basic.Label) {
        item.setText(item.getText());
        item.setFontSize(item.getFontSize());
      }
    }
  },
